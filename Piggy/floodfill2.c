...
static void doit(char **tab, t_point size, t_point current, char filler) 
{
    if (current.y < 0 || current.y >= size.y || current.x < 0 || current.x >= size.x)
        return;
    if (tab[current.y][current.x] != '0' && tab[current.y][current.x] != 'P' && tab[current.y][current.x] != 'C' && tab[current.y][current.x] != 'E')
        return;
...
// Check if the map is impossible
void impossible_map2(t_game *game) 
{
    char **map = game->map;
    ...
    while (i < map_hight) 
	{
        ...
        copy[i][map_lenght] = '\0';  // Ensure null-terminated strings
        i++;
    }
    ...
    // Check if all 'C' and 'E' are reachable
    i = 0;
    while (i < map_hight) 
	{
        int j = 0;
        while (j < map_lenght) 
		{
            if ((map[i][j] == 'C' || map[i][j] == 'E') && copy[i][j] != 'F') 
			{
